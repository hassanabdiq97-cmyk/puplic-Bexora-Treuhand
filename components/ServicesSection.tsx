          ...
          {dict.servicesList.map((service: any, index: number) => {
            ...
            return (
              <div key={service.id} className={`flex flex-col h-full group animate-fade-up ${delayClass}`}>
                <div className={`flex-grow p-8 md:p-14 rounded-[2.5rem] md:rounded-[3rem] bg-slate-50 dark:bg-slate-900/40 border-2 transition-all duration-500 shadow-xl flex flex-col items-center text-center ${
                  ...
                  <div className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center mb-8 md:mb-10 transition-all duration-500 group-hover:scale-110 shadow-lg ${
                  ...
                  <div className="w-full mt-auto space-y-6 md:space-y-8">
                    <div className="flex flex-wrap justify-center gap-2 md:gap-3">
                      {service.details.map((detail: string, idx: number) => (
                        <span key={idx} className="text-[8px] md:text-[9px] px-2.5 py-1.5 md:px-3 rounded-lg bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-200 font-bold uppercase tracking-widest">{detail}</span>
                      ))}
                    </div>
                    {/* Explicit minimum height for touch area */}
                    <MagneticButton variant="outline" className="!w-full !py-4 md:!py-5 !min-h-[50px] md:!min-h-[56px] !text-[10px] !rounded-2xl group-hover:!bg-slate-900 dark:group-hover:!bg-white group-hover:!text-white dark:group-hover:!text-slate-900 group-hover:!border-transparent" onClick={() => onOpenCalculator(service.color === 'amber' ? 'private' : 'business')}>
                      {t.cta}
                    </MagneticButton>
                  </div>
                </div>
