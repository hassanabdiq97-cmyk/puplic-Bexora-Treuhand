...
import { ArrowUpRight, MapPin, Clock, X, CheckCircle2, ArrowRight } from 'lucide-react';
...
const CareersSection: React.FC<CareersSectionProps> = ({ t, onApplyClick }) => {
  const content = t.sections.careers;
  ...
  const handleApply = () => {
    closeJob();
    // Small timeout to allow modal animation to start closing before scrolling
    setTimeout(() => {
      onApplyClick();
    }, 100);
  };
    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start relative z-10 py-10">
        ...
        <div className="lg:col-span-7 space-y-6 animate-up">
          ...
          <div className="p-10 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-white/5 flex flex-col items-center justify-center text-center space-y-4 hover:border-blue-600/20 transition-colors bg-slate-50/50 dark:bg-transparent">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-500 max-w-xs leading-relaxed">{content.emptyDesc}</p>
            <button onClick={onApplyClick} className="group flex items-center gap-2 text-blue-600 font-black uppercase text-[10px] tracking-widest hover:text-blue-500 transition-colors">
              Kontakt aufnehmen <span className="group-hover:translate-x-1 transition-transform">→</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 md:p-12 animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-4xl bg-white dark:bg-[#0d1425] rounded-[3rem] shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-8 duration-500">
              ...
              <div>
                <div className="flex gap-2 mb-4">
                  {selectedJob.tags.map((tag: string) => (
                    <span key={tag} className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 bg-blue-600/10 text-blue-600 rounded-md">{tag}</span>
                  ))}
                </div>
                <h3 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight">{selectedJob.title}</h3>
                <div className="flex items-center gap-6 mt-4 text-slate-500 text-[10px] font-black uppercase tracking-widest">
                  <span className="flex items-center gap-2"><Clock size={14}/> {selectedJob.pensum}</span>
                  <span className="flex items-center gap-2"><MapPin size={14}/> {selectedJob.location}</span>
                </div>
              </div>
              <button onClick={closeJob} className="p-4 rounded-full bg-slate-100 dark:bg-white/5 text-slate-500 hover:text-blue-600 transition-colors">
                <X size={24} />
              </button>
            </div>
                ...
                <div className="space-y-6">
                  ...
                  <ul className="space-y-4">
                    {selectedJob.tasks.map((task: string, i: number) => (
                      <li key={i} className="flex gap-4 text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
              ...
              <div className="bg-blue-600/5 rounded-3xl p-8 border border-blue-600/10">
                <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                  <div className="max-w-md text-left">
                    ...
                    <p className="text-sm text-slate-500">Wir freuen uns darauf, Sie persönlich kennenzulernen. Senden Sie uns eine Nachricht oder rufen Sie uns direkt an.</p>
                  </div>
                  <MagneticButton variant="primary" className="!px-10 !py-4" onClick={handleApply}>
                    {content.apply} <ArrowRight size={18} />
                  </MagneticButton>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 text-center bg-slate-50 dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/5">
              <button onClick={closeJob} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
                {content.close}
              </button>
            </div>
          </div>
